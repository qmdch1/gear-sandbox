...
const BUCKET_REST_Y = -1; // bucket down at the water, i.e. the CENTRE of the bucket body
const BUCKET_H = 3.4; // height of the bucket body; its rim sits BUCKET_H / 2 above its centre
const BARREL_CLEARANCE = 1; // air left between the raised bucket's rim and the barrel above it
/** X of the gears' shared axle (see `createWellPumpPreset`) and of the two ends of the axle
 *  stub that joins them to the barrel: just outboard of the gear discs, and just inside the
 *  barrel's end face at x = -WELL_R. */
const GEAR_X = -WELL_R - 3;
const AXLE_OUTBOARD_X = GEAR_X - 0.4;
const AXLE_INBOARD_X = -WELL_R + 0.1;

/** Effective radius the rope spools at on the windlass drum -- and the radius of the barrel
 *  prop the rope visibly wraps, so the two cannot drift apart. Rope-on-drum kinematics give
 *  the bucket `drumRotation * ROPE_RADIUS` of lift (the same angle x radius relation
 *  `rotation.ts` uses to drive a rack from a pinion). */
export const ROPE_RADIUS = 2.2;

/** How far the bucket travels between the water and the windlass. DERIVED from where the
 *  barrel actually is rather than hand-picked, so the top of the stroke parks the bucket's
 *  RIM just under the barrel: the barrel's underside is `DRUM_Y - ROPE_RADIUS` (19.8) and the
 *  rim starts at `BUCKET_REST_Y + BUCKET_H / 2` (0.7), leaving 19.1 of headroom before the
 *  bucket touches wood, minus a hand's width of clearance.
 *
 *  A hand-written 20 here -- what this preset shipped with -- overshot that by 0.9: the rim
 *  finished every stroke at y = 20.7 against a barrel underside of 19.8, so the bucket ended
 *  each draw visibly buried inside the windlass barrel (and the iron hoop with it), while the
 *  test suite, which only ever compared the lift against BUCKET_TRAVEL itself, called it
 *  exact. Deriving it makes the clash impossible to reintroduce by editing DRUM_Y or the
 *  bucket's size, and `tests/sim/presets/wellpump.test.ts` now asserts the clearance directly. */
export const BUCKET_TRAVEL =
  DRUM_Y - ROPE_RADIUS - (BUCKET_REST_Y + BUCKET_H / 2) - BARREL_CLEARANCE;
 ...
 *  bucket's rim a clear BARREL_CLEARANCE below the windlass barrel rather than inside it (see
 *  BUCKET_TRAVEL). Verified numerically in tests/sim/presets/wellpump.test.ts. */
export function createWellPumpPreset(): LayoutState {
  const gears: GearInstance[] = [
    seedGear(HANDLE_ID, "crank", [GEAR_X, DRUM_Y, 0], [1, 0, 0], 12, 1, HANDLE_SPEED),
    seedGear(DRUM_ID, "pulley", [GEAR_X, DRUM_Y, 0], [1, 0, 0], 10, 1),
  ];
...
export function createWellPumpProps(): Prop[] {
  ...
  const props: Prop[] = [
    ...
    // The axle stub carrying that barrel out to the handle. Without it the crank and drum
    // gears -- which sit 3 units beyond the barrel's end face, at GEAR_X -- hang in
    // mid-air with nothing visibly joining them to the barrel they turn. Left un-attached
    // rather than spun with the drum on purpose: a plain cylinder is rotationally symmetric,
    // so spinning it would cost a per-frame transform and look identical.
    { kind: "cylinder", position: [(AXLE_OUTBOARD_X + AXLE_INBOARD_X) / 2, DRUM_Y, 0], radius: 0.55, height: AXLE_INBOARD_X - AXLE_OUTBOARD_X, color: iron, texture: "metal", rotation: [0, 0, Math.PI / 2], roughness: 0.4, metalness: 0.6 },
    ...
    // The bucket, and its iron hoop, both hoisted by the drum. Both are sized off BUCKET_H,
    // which is also what BUCKET_TRAVEL derives its stop from -- resize the bucket and the
    // stroke that keeps it clear of the barrel follows.
    { kind: "cylinder", position: [0, BUCKET_REST_Y, 0], radius: 2.4, height: BUCKET_H, color: bucketCol, texture: "wood", roughness: 0.75, metalness: 0.05, windWith: lift },
    { kind: "ring", position: [0, BUCKET_REST_Y + BUCKET_H / 2 - 0.3, 0], radius: 2.5, tube: 0.25, color: iron, texture: "rust", rotation: [Math.PI / 2, 0, 0], roughness: 0.7, metalness: 0.5, windWith: lift },
  ];
